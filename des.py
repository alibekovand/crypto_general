...
def encryption_block(user_input):
...
def decryption_block(final_cipher):
...
def encryption(user_input):
    cypher = ''.join(encryption_block(user_input[i:i+8]) for i in range(0, len(user_input), 8))
    return cypher


def decryption(user_input):
    cypher_text = ''.join(decryption_block(str_to_bin(user_input[i:i+8])) for i in range(0, len(user_input), 8))
    return cypher_text
